...
void CMainWnd::OnPaint(HDC hDC, PAINTSTRUCT* pps)
{
	...
	SetRect(&rc, INFO_LEFT, INFO_TOP, 
			INFO_LEFT + m_nInfoWidth, INFO_TOP + INFO_HEIGHT);
	if (NeedToRepaint(&pps->rcPaint, &rc))
		DrawInfo(m_hMemDC);
	...
	SetRect(&rc, PEEK_LEFT, PEEK_TOP, 
		PEEK_LEFT + m_nPeekWidth, PEEK_TOP + PEEK_HEIGHT);
	if (NeedToRepaint(&pps->rcPaint, &rc))
		DrawPeek(m_hMemDC);
...
void CMainWnd::OnLButtonDown(int x, int y)
{
	...
	if (x >= BTN_LEFT && y >= m_nBtnTop &&
		...
		SetVolume(MAKELONG((int)dPos, (int)dPos));
	...
	if (m_pSkin->GetImageList(IMAGELIST_BUTTON)) {
		...
		if (m_fPressed && m_nPressed == nIndex) {
			...
			ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_BUTTON), nIndex, hDC, 
				x + ((BTN_WIDTH - SCALEX(16)) / SCALEX(2)) + SCALEX(1), y + ((BTN_HEIGHT - SCALEY(16)) / SCALEY(2)) + SCALEY(1), ILD_TRANSPARENT);
		}
		else {
			ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_BUTTON), nIndex, hDC, 
				x + ((BTN_WIDTH - SCALEX(16)) / SCALEX(2)), y + ((BTN_HEIGHT - SCALEY(16)) / SCALEY(2)), ILD_TRANSPARENT);
		}
	}
	else {
		// 新しいバージョンのボタン描画
...
void CMainWnd::DrawTrack(HDC hDC)
{
	...
	ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_NUMBER), n % 10, hDC, TRACK_LEFT + SCALEX(10), TRACK_TOP, ILD_TRANSPARENT);
}
...
void CMainWnd::DrawTime(HDC hDC)
{
	...
	ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_NUMBER), n % 10, hDC, TIME_LEFT + SCALEX(10), TIME_TOP, ILD_TRANSPARENT);

	// Separator
	ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_NUMBER), 10, hDC, TIME_LEFT + SCALEX(20), TIME_TOP, ILD_TRANSPARENT);

	// Seconds
	n = nSec % 60;
	ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_NUMBER), n / 10, hDC, TIME_LEFT + SCALEX(24), TIME_TOP, ILD_TRANSPARENT);
	ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_NUMBER), n % 10, hDC, TIME_LEFT + SCALEX(34), TIME_TOP, ILD_TRANSPARENT);
}
...
void CMainWnd::DrawInfo(HDC hDC)
{
	...
	if (pInfo && pInfo->info.nSamplingRate && pInfo->info.nChannels) {
		TCHAR szInfo[128];
		RECT rc = {INFO_LEFT, INFO_TOP, INFO_LEFT + m_nInfoWidth, INFO_TOP + INFO_HEIGHT};
...
void CMainWnd::DrawPlayOption(HDC hDC)
{
	...
	if (m_pOptions->m_fShuffle) {
		ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_PLAYOPT), 1, hDC, PLAYOPT_LEFT, PLAYOPT_TOP + SCALEY(8), ILD_TRANSPARENT);
	}
}
...
void CMainWnd::DrawStatus(HDC hDC)
{
	...
	ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_STATUS), nIndex, hDC, STATUS_LEFT, 
									m_nStatusTop + (m_nStatusHeight - STATUS_WIDTH) - SCALEY(2), ILD_TRANSPARENT);
#else
	ImageList_Draw(m_pSkin->GetImageList(IMAGELIST_STATUS), nIndex, hDC, STATUS_LEFT, 
...
#define VOLBAR_LEFT		50
	...
	int nVolWidth = m_nTitleWidth - VOLBAR_LEFT;
	int nWidth = nVolWidth * dwVolume / 100;
	RECT rcBar = {TITLE_LEFT + VOLBAR_LEFT, m_nTitleTop + (m_nTitleHeight - size.cy) / 2 + SCALEY(4),
				TITLE_LEFT + VOLBAR_LEFT + nWidth, m_nTitleTop + m_nTitleHeight - (m_nTitleHeight - size.cy) / 2};
...
{
	if (m_pOptions->m_fDrawPeek) {
		int n = min(m_nPeekWidth * m_nPeek[0] / 100, m_nPeekWidth);
		RECT rcLeft = {PEEK_LEFT, PEEK_TOP, PEEK_LEFT + n, PEEK_TOP + SCALEY(2)};
		HBRUSH hbr = CreateSolidBrush(m_pSkin->GetColor(COLOR_PEEKL));
		FillRect(hDC, &rcLeft, hbr);
		DeleteObject(hbr);

		n = min(m_nPeekWidth * m_nPeek[1] / 100, m_nPeekWidth);
		RECT rcRight = {PEEK_LEFT, PEEK_TOP + SCALEY(3), PEEK_LEFT + n, PEEK_TOP + SCALEY(5)};
...
DWORD CMainWnd::GetVolume()
{
	return MAP_GetVolume(m_hMap, m_pOptions->m_fUseSysVolume);
}

void CMainWnd::SetVolume(DWORD dwVolume)
{
	MAP_SetVolume(m_hMap, dwVolume, m_pOptions->m_fUseSysVolume);
}
