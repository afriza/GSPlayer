	...
	case WM_NOTIFY:
		{
			...
			case LVN_ITEMCHANGING:
				return pMainWnd->OnListViewItemChanging((NMLISTVIEW*)pnmh);
			default:
				return CMainWnd::MainWndProc(hwnd, uMsg, wParam, lParam);
			}
		}
...
BOOL CPpcMainWnd::InitImages()
{
	...
	if (fRet) {
		...
		SendMessage(m_hwndLV, WM_SETFONT, (WPARAM)m_hFntTitle, 0);
...
void CPpcMainWnd::InitSize()
{
	...
	m_nMenuHeight = rc.top == 0 ? 0 : MENU_HEIGHT;
...
LRESULT CPpcMainWnd::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case IDM_PLAY_PLAYSTOP:
		OnPlayPlayStop();
		return 0;
	case IDM_TOOL_KEYCTRL:
		OnToolKeyCtrl();
		return 0;
	case IDM_FILE_SELMODE:
		OnFileSelMode();
		return 0;
	}
...
void CPpcMainWnd::OnInitMenuPopup(HMENU hMenu, UINT uPos, BOOL fSystemMenu)
{
	...
	CheckMenuItem(hMenu, IDM_FILE_SELMODE,
		m_bListSelecting ? MF_BYCOMMAND | MF_CHECKED : MF_BYCOMMAND | MF_UNCHECKED);
}
...
void CPpcMainWnd::ExchangeItem(HWND hwndLV, int nItem1, int nItem2)
{
	BOOL bSelecting = m_bListSelecting;
	m_bListSelecting = FALSE;
...
void CPpcMainWnd::OnListViewKeyDown(NMHDR* pnmh)
{
	...
	if (((LPNMLVKEYDOWN)pnmh)->wVKey == VK_LEFT)
		OnPlayRew();
	else if (((LPNMLVKEYDOWN)pnmh)->wVKey == VK_RIGHT)
		OnPlayFf();
	else if (((LPNMLVKEYDOWN)pnmh)->wVKey == VK_SPACE) {
		if (m_bListSelecting)
			ListSelectToggle();
	}
}
...
void CPpcMainWnd::OnListViewConextMenu(NMHDR* pnmh)
{
	...
	int nCount = GetListSelectedCount();
	if (nCount) {
		HMENU hMenu, hSub;
...
void CPpcMainWnd::OnToolKeyCtrl()
{
	UnregisterHotKeys();
	if (!m_pKeyCtrlDlg) {
		m_pKeyCtrlDlg = new CPpcKeyCtrlDlg;
		m_pKeyCtrlDlg->ShowKeyCtrlDlg(m_hWnd, m_hwndLV, ((CPpcSkin*)m_pSkin)->GetColor2(COLOR2_PLAYLISTBACK), ((CPpcSkin*)m_pSkin)->GetColor2(COLOR2_PLAYLISTTEXT));
		delete m_pKeyCtrlDlg;
		m_pKeyCtrlDlg = NULL;
	}
	RegisterHotKeys();
}

int CPpcMainWnd::GetListSelectedCount()
{
	int nCount = ListView_GetItemCount(m_hwndLV);
	int nSelected = 0;
	for (int i = 0; i < nCount; i++) {
...
void CPpcMainWnd::OnFileSelMode()
{
	m_bListSelecting = !m_bListSelecting;
}

BOOL CPpcMainWnd::OnListViewItemChanging(NMLISTVIEW* pnmlv)
{
	if (!m_bListSelecting)
		return FALSE;
...
void CPpcMainWnd::ListSelectToggle()
{
	if (!m_bListSelecting)
		return;
